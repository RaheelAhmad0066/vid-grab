...
import { 
  ...
  Moon, Sun, Share2, Trash2, Settings, History, Copy
} from 'lucide-react'
...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('vidgrab-history')
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (history.length > 0) {
      localStorage.setItem('vidgrab-history', JSON.stringify(history))
    }
  }, [history])
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter' && url && status === 'idle') {
        handleAnalyze()
      }
      // Escape to reset
      if (e.key === 'Escape') {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        ...
        setStatus('error')
        toast.error(data.error || 'Could not fetch video info', { id: 'analyze' })
        return
      }
      setVideoInfo(data)
      ...
      setStatus('error')
      toast.error('Backend not reachable', { id: 'analyze' })
    }
  ...
  const handleShare = async () => {
    if (navigator.share && videoInfo) {
      try {
        await navigator.share({
          title: videoInfo.title,
          text: `Check out: ${videoInfo.title}`,
          url: url
        })
  ...
  const handleClearHistory = () => {
    setHistory([])
    localStorage.removeItem('vidgrab-history')
        ...
        >
          ...
          <div className="input-section">
            <div className={`input-wrapper ${detectedPlatform ? 'has-platform' : ''} ${status === 'error' ? 'has-error' : ''}`}>
              ...
              {url && (
                <>
                  <button className="clear-btn" onClick={handleCopyLink} title="Copy link">
                    <Copy size={18} />
                  </button>
